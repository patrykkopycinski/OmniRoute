...
import { applyComboStepParams, applyComboStepResponseGuards } from "./combo/stepParams.ts";
import {
      ...
      for (let setTry = 0; setTry <= maxSetRetries; setTry++) {
        ...
        const executeTarget = async (
          ...
          for (let retry = 0; retry <= maxRetries; retry++) {
            ...
            // feat/combo-step-params: per-step request shaping (token cap /
            // thinking off / extra_body merge) on BOTH dispatch paths. Applied
            // AFTER the reasoning-buffer block so explicit caps always win.
            if (target.params) {
              applyComboStepParams(
                attemptBody as unknown as Record<string, unknown>,
                target.params
              );
            }
